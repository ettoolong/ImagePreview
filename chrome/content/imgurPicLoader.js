function EttImgurPicLoader(listener) {
  this.listener = listener;
  this.dp = new DOMParser();
}

EttImgurPicLoader.prototype={
  lastUrl: '',
  lastSuccessUrl: '',
  lastSuccessPic: '',
  ImgurRegEx: /^(https?:\/\/imgur\.com\/)(\w{5,8})(\?tags)?/i,
  codeBlockList: [],
  codeQueue: [],
  codePicture: [],
  maxBlock: 150,
  ...
  load: function(url){
    ...
    if(this.listener && this.ImgurRegEx.test(url))
    {
      //we need block some url at here!
      //var consoleService = Components.classes["@mozilla.org/consoleservice;1"].getService(Components.interfaces.nsIConsoleService);
      //consoleService.logStringMessage("B=" + this.codeBlockList.length + ", Q=" + this.codeQueue.length + ", P =" + this.codePicture.length);
      var strArr = url.split(this.ImgurRegEx);
      if(strArr.length<3)
        return false;
      //var shortCode = strArr[2];
      
      //this.listener.setPicLocation(this.listener.view.tempPicX, this.listener.view.tempPicY);
      if(this.lastSuccessUrl==url)
      {
        this.listener.showPicPreview(this.lastSuccessUrl, this.lastSuccessPic);
          return true;
      }
      ...
      for(var i in this.codePicture)
      {
        ...
        {
          this.listener.showPicPreview(this.codePicture[i].url, this.codePicture[i].pic);
          return true;
        }
      }
      if(this.lastUrl!=url)
      {
        ...
        this.lastUrl = url;
        var xmlhttp = new XMLHttpRequest();
        xmlhttp.onreadystatechange = function() {EttImagePreview.imgurPicLoader.onPageResponse(this);};
        xmlhttp.open("GET",url,true);
        xmlhttp.channel.QueryInterface(Components.interfaces.nsIHttpChannel).redirectionLimit = 0;
        xmlhttp.UniUrl = url;
        xmlhttp.LoaderAction = 'load';
        xmlhttp.owner = this.listener;
        xmlhttp.send(null);
      }
      return true;
    }
